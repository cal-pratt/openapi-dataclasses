...
from .formatter import property2attribute, enum2attribute
from .reference import PythonContext
from ..types.openapi import OpenApiSpec, OpenApiSchema
from ..types.python import PythonImport, PythonClassAttribute, PythonEnumAttribute
...
def find_attribute_type(
    *,
    ...
    # find_references should check to see if a union is needed
        ...
        if schema.type is not None and len(schema.type) > 0 and "array" not in schema.type:
...
def update_model_attributes(
    ...
    for current_context in class_contexts.values():
        ...
        if schema.enum and schema.properties:
            raise ValueError(f"Component Schema is not a valid enum or object type: {schema}")
